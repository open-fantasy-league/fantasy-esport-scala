...
class TeamRepoImpl @Inject()()(implicit ec: TeamExecutionContext, leagueRepo: LeagueRepo) extends TeamRepo{
  ...
  override def cardsInTeam(cardIds: List[Long], currentPeriod: Option[Int])(implicit c: Connection): Boolean = {
    SQL"""select 1 as yeppers from team where card_id = ANY(ARRAY[$cardIds]::bigint[]) AND ($currentPeriod IS NULL OR upper(timespan) is NULL or upper(timespan) >= $currentPeriod)""".
